...
if [ "${DOCKER_RUN_AS_GID}" != "0" ]; then
  ...
  crudini --verbose --set /etc/php/7.3/fpm/pool.d/www.conf www group "${DOCKER_RUN_AS_GID}"
fi
if [ "${DOCKER_RUN_AS_UID}" != "0" ]; then
  ...
  crudini --verbose --set /etc/php/7.3/fpm/pool.d/www.conf www user "${DOCKER_RUN_AS_UID}"
fi
...
do
  ...
  crudini --verbose --set "/etc/php/7.3/fpm/php-fpm.conf" "${ENVVAR_SECTION}" "${ENVVAR_KEY}" "${ENVVAR_VALUE}"
done

PHP_INI_CONFIGURATION_FILE="$(php-fpm7.3 -i | grep '^Loaded Configuration File' | awk '{print $5}')"
...
do
  ...
  ENVVAR_KEY=$(echo ${ENVVAR} | cut -d '=' -f2 | cut -d ':' -f 2)
  ENVVAR_VALUE=$(echo ${ENVVAR} | cut -d '=' -f2 | cut -d ':' -f 3-)
  crudini --verbose --set "/etc/php/7.3/fpm/pool.d/www.conf" "${ENVVAR_SECTION}" "${ENVVAR_KEY}" "${ENVVAR_VALUE}"
done

php-fpm7.3 --test

exec /usr/sbin/php-fpm7.3 -R --nodaemonize --fpm-config /etc/php/7.3/fpm/php-fpm.conf
